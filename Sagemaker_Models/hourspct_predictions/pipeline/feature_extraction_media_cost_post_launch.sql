...
COPY INTO {stage}/hourspct/media_cost_postlaunch_feature.csv
FROM (
    with title_id as (
        select distinct
            title_name
    ...
    total_mk_spend as (
        ...
        group by 1, 2
        order by match_id, days_since_first_offered,
    ...
    pivot_base as (
            select
                ...
                , sum(total_media_cost_from_marketing_spend) over
                    (partition by match_id order by days_since_first_offered)
