...
PIPELINE_LOGGER_SCRIPT = (
    "/dags/resources/logging/max_event_processing_execution_log.sql"
)
    ...
    attachments = [
        {
            ...
            else "The below task has *succeeded* for today's run \n*TASK:* {}".format(
                context["task"].task_id
            ),
...
def create_dag(dag_config):
    ...
    try:
        ...
        start_date = datetime.strptime(
            Variable.get(dag_config["default_args"]["start_date"]), "%Y-%m-%d"
        )
        if "start_date_delta_days" in dag_config["default_args"]:
            start_date = start_date + timedelta(
                days=dag_config["default_args"]["start_date_delta_days"]
            )
    ...
    if "end_date" in dag_config["default_args"]:
        try:
            ...
            end_date = datetime.strptime(
                Variable.get(dag_config["default_args"]["end_date"]), "%Y-%m-%d"
            )
    else:
        end_date = None
    ...
    def slack_success(context):
        response = (
            requests.post(
                ...
                data=json.dumps(
                    {
                        "channel": channel,
                        "username": "Sagemaker Job Alerts",
                        "text": "*DAG ALERT:{} - SUCCESS*".format(
                            context["dag"].dag_id
                        ),
                        "attachments": slack_attachment("success", context),
                    }
                ),
            )
        ...
        if (response is not None) and response.status_code != 200:
            print(
                "Unable to invoke slack api status code= ",
                ...
                data=json.dumps(
                    {
                        "channel": channel,
                        "username": "Sagemaker Job Alerts",
                        "text": "*DAG ALERT:{} - FAILED*".format(context["dag"].dag_id),
                        "attachments": slack_attachment("failed", context),
                    }
                ),
            )
            if ((channel is not None) and (len(channel) > 0))
                ...
                data=json.dumps(
                    {
                        "channel": channel,
                        "username": "Sagemaker Job Alerts",
                        "text": "*DAG ALERT:{} - RETRY*".format(context["dag"].dag_id),
                        "attachments": slack_attachment("retry", context),
                    }
                ),
            )
            if ((channel is not None) and (len(channel) > 0))
    ...
    default_args = {
        ...
        "end_date": end_date,
        "email": [dag_config["default_args"]["email_prod"]]
        if ENV == "PROD"
        else [dag_config["default_args"]["email_nonprod"]],
    ...
    dag = DAG(
        ...
        if ("None" != dag_config.get("schedule_interval"))
        else None,
        catchup=dag_config.get("catchup")
        if ("None" != dag_config.get("catchup"))
        else False,
        default_args=default_args,
        max_active_runs=dag_config.get("max_active_runs")
        if ("None" != dag_config.get("max_active_runs"))
        else configuration.conf.getint("core", "max_active_runs_per_dag"),
    )
    with dag:
        dag_dict[dag.dag_id] = {}
        last_task = None
        for task_loop_index, task_details in enumerate(dag_config["tasks"], 0):
            last_task = create_task(
                dag,
                dag_config["tasks"][task_loop_index],
                dag_config["tasks"],
                dag_config,
                str(dag_config["default_args"]["owner"]),
                dag_config["schema"],
                dag_config["snowflake_config"],
                dag_config["dynamo_cred_tbl"],
                dag_config.get("model_name", dag_config.get("glue_model_name")),
            )
            dag_dict[dag.dag_id][
                dag_config["tasks"][task_loop_index]["name"]
            ] = last_task
...
def generate_looped_template_dict_sql(task_details, pipeline_dir, templates_dict):
    ...
    sql_loop_seed = (
        task_details["sql_loop_seed"]
        if "sql_loop_seed" in task_details
        else DEFAULT_SQL_LOOP_SEED
    )
    sql_loop_step = (
        task_details["sql_loop_step"]
        if "sql_loop_step" in task_details
        else DEFAULT_SQL_LOOP_STEP
    )
...
def get_task(dag, task_id):
    ...
    if task is None:
        print("Error: Task Not Found dag_id= ", dag.dag_id, " and task_id= ", task_id)
    return task
...
def create_task(
    ...
    if "type" in task_details and task_details["type"] == "BashOperator":
        ...
        return create_bash_operator_task(
            dag, task_details, all_tasks, dag_config, owner
        )
        ...
        def _python_func(**kwargs):
            ...
            sql_schema = (
                task_details.get("sql_schema")
                if ("None" != task_details.get("sql_schema"))
                else sql_schema_dag_level
            )

            if dynamo_cred_tbl is None:
                conn = su.connect(
                    SNOWFLAKE_ACCOUNT_NAME,
                    snowflake_config["snowflake_db_name"],
                    sql_schema,
                    snowflake_config["snowflake_warehouse_name"],
                    snowflake_config["snowflake_etl_role"],
                    snowflake_config["snowflake_environment_name"],
                )
            else:
                conn = su.connect(
                    SNOWFLAKE_ACCOUNT_NAME,
            ...
            if "sql_loop" in task_details:
                ...
                for sql_file_name in task_details["sql_file_names"]:
                    pipeline_sequence += [
                        [pipeline_dir + sql_file_name, pipeline_dir + sql_file_name]
                    ]

                pipeline_sequence += [[PIPELINE_LOGGER_SCRIPT, PIPELINE_LOGGER_SCRIPT]]
                # print(pipeline_sequence)

                su.execute_sql_pipeline_sequence(
                    "",
                    pipeline_sequence,
                    conn,
                    dag_id=dag.dag_id,
        ...
        priority_weight = (
            task_details["priority_weight"]
            if "priority_weight" in task_details
            else DEFAULT_PRIORITY_WEIGHT
        )
        depends_on_past = (
            task_details["depends_on_past"]
            if "depends_on_past" in task_details
            else DEFAULT_DEPENDS_ON_PAST
        )
...
def create_bash_operator_task(dag, task_details, all_tasks, dag_config, owner):
    ...
    if "glue_job_processing_script" in task_details["name"]:
        ...
        param_file_name = (
            task_details["Details"].get("param_file_name", "parameters") + ".json"
        )
        req_file_name = (
            task_details["Details"].get("requirements_file_name", "requirements")
            + ".txt"
        )
        ...
        command = """cd %s
        python3 glue_job_processor.py --model "%s" --script_name "%s"  --param_file "%s" --requirements_file_name "%s" --glue_version "%s" --worker_type "%s" --number_of_workers "%s" --job_type "%s" --env "%s" --owner_name "%s" --bucket "%s" --script_loc "%s" --dag_details "%s" --timeout "%s" --airflow_repo "%s" --dag_id "%s" --task_name "%s" """ % (
            str(FRAMEWORK_PATH),
            str(dag_config["glue_model_name"]),
            str(task_details["Details"]["script_name"]),
            param_file_name,
            req_file_name,
            "2.0",
            str(task_details["Details"]["worker_type"]),
            str(task_details["Details"]["number_of_workers"]),
            "Spark",
            str(ENV_LOWER),
            str(owner),
            str(bucket),
            task_details["Details"]["script_loc"],
            dag_details,
            str(timeout),
    ...
    if "sagemaker_notebook_job_processing_script" in task_details["name"]:
        ...
        if ENV_LOWER == "dev":
            ...
            bucket = task_details["BucketIOConfig"]["PROD"]["model_bucket"]
        param_file_name = task_details.get("param_file_name", "parameters") + ".json"
        dag_details = task_details.get("templates_dict", {})
        volume_size = task_details["Details"].get("volume_size_InGB", 50)
        command = """cd %s
        python3 sg_job_processor.py --instance_type "%s" --instance_count "%s" --bucket "%s" --model "%s" --notebook_name "%s" --owner_name "%s" --env "%s" --param_file "%s" --kernel "%s" --volume_size_InGB "%s" --dag_details "%s" --airflow_repo "%s" --dag_id "%s" --task_name "%s" """ % (
            str(FRAMEWORK_PATH),
            str(task_details["Details"]["instance_type"]),
            task_details["Details"]["instance_count"],
            str(bucket),
            str(dag_config["model_name"]),
            str(task_details["notebook_name"]),
            str(owner),
            str(ENV_LOWER),
            param_file_name,
            kernel,
    ...
    if task_details["name"].startswith("python_script"):
        if (
            task_details["script_loc"] == "model"
            and task_details.get("model_type") == "sagemaker"
        ):
            model_root = "Models"
            ...
            task_details["script_loc"] == "model"
            and task_details.get("model_type") == "glue"
        ):
        ...
        # params={}
        def_path = "{}/{}/{}{}".format(file_path, model_root, model, base_folder)
        param_file_loc = "{}/{}.json".format(def_path, task_details["param_file_name"])
        params_str = ""
        dag_details = task_details.get("templates_dict", {})
        if os.path.exists(param_file_loc):
            with open(param_file_loc) as f:
                data = json.load(f)
                params_temp = data.get(ENV.upper(), data.get(ENV.lower(), data))
    ...
    bash_Operator_task = BashOperator(
        task_id=task_details["name"], bash_command=command, dag=dag
    )
...
def create_external_task_sensor(dag, task_details):
    ...
    :return:
    """
    external_task = task_details["external_dag_details"].get("external_task_id", None)
    poke_interval = task_details["external_dag_details"].get("poke_interval", 60)
    ...
    external_dag_task_sensor = ExternalTaskSensor(
        ...
        if str(external_task).lower() != "none"
        else None,
        allowed_states=task_details["external_dag_details"]
        .get("allowed_states", "success")
        .split(","),
        poke_interval=eval(poke_interval)
        if isinstance(poke_interval, str)
        else poke_interval,
        mode=mode,
...
def get_sf_config(config):
    ...
    if "db_details" in config:
        ...
        sf_config = {
            "snowflake_db_name": config["snowflake_db_name"],
            "snowflake_environment_name": config["snowflake_environment_name"],
            "snowflake_etl_role": config["snowflake_etl_role"],
            "snowflake_warehouse_name": config["snowflake_warehouse_name"],
        }
    return sf_config


yaml.add_constructor("!include", yml_includer.construct_include, yml_includer.Loader)
        ...
        try:
            ...
            for name, dag_config in dags.items():
                ...
                try:
                    ...
                    dag_config["dynamo_cred_tbl"] = (
                        dag_config.get("dynamo_cred_tbl")
                        if "dynamo_cred_tbl" in dag_config
                        else None
                    )
